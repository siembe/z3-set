...
export const WebRTCProvider = ({children}) => {
    ...
    async function handleMessage({data}) {
        ...
        if (data.senderId === id.current) {
            console.log("🛑 Ignoring echoed update from self");
            return;
        }
    ...
    function sendUpdate(update) {
        ...
        onDataReceived.current.forEach(callback => callback(payload));
    }

    function subscribe(callback) {
        onDataReceived.current.push(callback);
        return () => {
            onDataReceived.current = onDataReceived.current.filter(cb => cb !== callback);
        }
    }
    ...
    return (
        <WebRTCContext.Provider value={{
            subscribe,
            onDataChannelSetup: onDataChannelSetup.current,
            host,
            join,
            sendUpdate
        }}>
